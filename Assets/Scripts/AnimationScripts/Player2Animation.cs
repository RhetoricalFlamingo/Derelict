...
public class Player2Animation : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetAxis("RHorizontal") > 0.2) //if player is walking right
		{
		...
		else if (Input.GetAxis("RHorizontal") < -0.2)	//if player is walking left
		{
			_anim.SetBool("walking", true);
			_anim.SetBool("looking_side", true);
			_anim.SetBool("looking_front", false);
		...
		if (Input.GetAxis("RVertical") > 0.2) //if player is walking down
		{
			_anim.SetBool("walking", true);
		...
		else if (Input.GetAxis("RVertical") < -0.2)
		{
			_anim.SetBool("walking", true);
			_anim.SetBool("looking_back", false);
			_anim.SetBool("looking_side", false);
